...
class SmoothnessRegularizedLoss(nn.Module):
    ...
    L_total = L_MSE + λ_s·L_strike + λ_e·L_expiry + λ_c·L_calendar + λ_b·L_butterfly + λ_m·|E[r]|²
    """
    def __init__(
        self, 
        ...
        lambda_residual_mean: float = 0.0,
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor, residual: torch.Tensor = None) -> tuple:
        """
        Args:
            y_pred: Predicted forecast of shape (B, h, M, N)
            y_true: True surface of shape (B, h, M, N)
            residual: Optional raw LSTM residual tensor (B, h, M, N) for zero-mean penalty
        Returns:
        ...
        # 2. Strike smoothness (dim=-1, kappas) with reflection padding
        ...
        # 3. Expiry smoothness (dim=-2, taus) with reflection padding
